% Note that Lp and w are held constant for these 5-a,3-b,1-c: paper
...
% ylabel('log(P_{i} - P_e)','FontSize',11,'FontWeight','bold');
grid on;
% legend('log(P_i - P_e) - Analytical','log(P_i - P_e) - Computational')
%Use the line handles to set the LineStyle properties of the left- and right-side plots:
set(HA1,'LineStyle','--','LineWidth',1.5)
set(HA2,'LineStyle',':','marker','o','LineWidth',1.5)
hold on;
%Ylabels
set(get(AX(1),'Ylabel'),'String','log(P_i - P_e) - Analytical','FontSize',10,'FontWeight','bold');
set(get(AX(2),'Ylabel'),'String','log(P_i - P_e) - Computational','FontSize',10,'FontWeight','bold');%'$Flow\ Unevenness={}^{{{u}_{MAX-Channel}}-{{u}_{HagenPoiseuille}}}/{}_{{{u}_{inlet}}}$', 'Interpreter', 'Latex')
